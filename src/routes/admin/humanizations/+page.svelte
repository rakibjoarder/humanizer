<script lang="ts">
	...
	function allOnPageSelected(): boolean {
		const ids = (data.humanizations ?? []).map((h: { id: string }) => h.id);
		return ids.length > 0 && ids.every((id) => selected.has(id));
	}

	function toggleAllOnPage(checked: boolean) {
		const next = new Set(selected);
		for (const h of data.humanizations ?? []) {
	...
	<div style="background: var(--color-bg-surface); border: 1px solid var(--color-bg-border); border-radius: 12px; overflow: hidden; overflow-x: auto;">
			...
			<div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
				<form
					...
					onsubmit={(e) => {
						...
						if (!confirm(`Delete ${selected.size} humanization(s)? This cannot be undone.`)) {
							e.preventDefault();
						}
					}}
				>
				...
				<form
					...
					action="?/deleteAll"
					onsubmit={(e) => {
						if (!confirm('Delete ALL humanizations? This cannot be undone.')) {
							e.preventDefault();
							return;
						}
		...
		<table style="width: 100%; border-collapse: collapse; font-family: 'Space Grotesk', system-ui; font-size: 13px;">
			...
			<tbody>
					...
					<tr style="border-bottom: 1px solid var(--color-bg-border);">
						<td style="padding: 10px 14px;">
							<input
								...
								onchange={(e) => toggleRow(h.id, (e.currentTarget as HTMLInputElement).checked)}
								aria-label="Select humanization"
							/>
						</td>
				...
				{:else}
					<tr><td colspan="5" style="padding: 32px; text-align: center; color: var(--color-text-muted);">No humanizations found.</td></tr>
				{/each}
			</tbody>
		</table>
	</div>
