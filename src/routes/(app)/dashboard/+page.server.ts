...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const { data: usageRow } = await locals.supabase
		.from('usage_logs')
		.select('words_used')
		.eq('user_id', user.id)
		.eq('date', todayUtc)
	...
	const wordsUsedToday = (usageRow?.words_used as number) ?? 0;
	const dailyLimit = profile.plan === 'free' ? 500 : -1;
	...
	// ── Resets in (ms until next midnight UTC) ────────────────────────────────
	const now = new Date();
