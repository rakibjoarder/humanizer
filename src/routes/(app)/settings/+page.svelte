			...
			<div class="space-y-2">
				{#each timeline as entry}
					{#if entry.kind === 'cancellation'}
						<div style="display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 16px; border-radius: 8px; background: #ef444408; border: 1px solid #ef444425;">
					...
					{:else}
						<div style="padding: 10px 16px; border-radius: 8px; background: var(--color-bg-elevated); border: 1px solid var(--color-bg-border);">
							...
							<p class="text-xs" style="color: var(--color-text-muted); margin: 4px 0 0;">{formatDate(entry.created)}</p>
						</div>
					{/if}
				{/each}
			</div>
